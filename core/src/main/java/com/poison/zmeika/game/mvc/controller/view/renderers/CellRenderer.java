...
public class CellRenderer extends GameObject {
    ...
    float time = 3.0f;
    TweenEquation equation = TweenEquations.easeInOutExpo;
    ...
    private void draw(SpriteBatch spriteBatch, float scaleX, float scaleY, float x,
                      float y, float ancorX, float ancorY){
    ...
    public void construct() {
        ...
        TweenController.start(Tween.to(innerScale, Vec2fTweenAccessor.POSITION_XY, time)
                        .target(1.0f, 1.0f).setCallbackTriggers(TweenCallback.END)
        ...
        TweenController.start(Tween.to(outerScale, Vec2fTweenAccessor.POSITION_XY, time)
                              .target(16.0f, 16.0f)
